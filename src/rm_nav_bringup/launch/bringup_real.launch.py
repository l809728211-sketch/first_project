...
def generate_launch_description():
   ...
   bringup_LIO_group = GroupAction([
        # 原有的变换：让全局 odom 对齐到 LIO 算法的起始原点
        Node(
            package="tf2_ros",
        ...
        # 【关键新增】：将雷达的位置 (lidar_odom) 反向推导回车体中心 (base_link)
        # 根据 XACRO，雷达在 base_link 前方 0.12m，上方 0.175m
        # 因此 base_link 在 lidar_odom 后方 -0.12m，下方 -0.175m
        Node(
            package="tf2_ros",
            executable="static_transform_publisher",
            arguments=[
                '-0.12', '0.0', '-0.175', '0.0', '0.0', '0.0',
                '--frame-id', 'lidar_odom',
